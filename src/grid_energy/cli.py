...
# Puzzle constants
DEFAULT_PUZZLE_SIZE = 5  # Changed from 12 to 5 for 5x5 puzzles only
...
def resolve_puzzle_with_retries(solver, energy_fn, hints, max_retries, puzzle_id, 
    ...
    for attempt in range(max_retries):
        tries_taken += 1
        torch.manual_seed(generate_seed_from_puzzle_id(puzzle_id, attempt))
        init_state = torch.randn((1, 1, DEFAULT_PUZZLE_SIZE, DEFAULT_PUZZLE_SIZE)).to(hints.device)
...
def resolve(
    ...
    max_retries: int = typer.Option(DEFAULT_MAX_RETRIES, help="Max attempts to find a logical solution"),
):
    ...
    # Get hint dimension from the sample
    ...
    best_resolved, min_errors, tries_taken, total_ms = resolve_puzzle_with_retries(
        solver, energy_fn, hints, max_retries, puzzle_id,
        puzzle_size, clean_row_hints, clean_col_hints
    )
